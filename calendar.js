...
/* ---------- modal ---------- */
function buildColorRow(containerId, selectedHex){
...
function openModal(ds){
  ...
  buildColorRow("colorRow", entry ? entry.hex : COLORS[0].hex);
...
/* ---------- range painting ---------- */
function eachDateStr(startStr, endStr, cb){
  let [sy,sm,sd] = startStr.split("-").map(Number);
  let [ey,em,ed] = endStr.split("-").map(Number);
  let cur = new Date(sy, sm-1, sd);
  const end = new Date(ey, em-1, ed);
  if(cur > end){ // swap if user picked them backwards
    const tmp = new Date(cur); cur = new Date(end); 
    end.setTime(tmp.getTime());
  }
  while(cur <= end){
    cb(dateStr(cur.getFullYear(), cur.getMonth(), cur.getDate()));
...
function openRangeModal(){
  ...
  document.getElementById("rangeActivity").value = "";
  document.getElementById("rangeNotes").value = "";
  buildColorRow("rangeColorRow", COLORS[0].hex);
  document.getElementById("rangeOverlay").classList.add("open");
}

function closeRangeModal(){
  document.getElementById("rangeOverlay").classList.remove("open");
}

function applyRange(){
  const start = document.getElementById("rangeStart").value;
  const end = document.getElementById("rangeEnd").value;
  const activity = document.getElementById("rangeActivity").value.trim();
  const notes = document.getElementById("rangeNotes").value.trim();
  const selectedDot = document.querySelector("#rangeColorRow .color-opt.selected");
  const hex = selectedDot ? selectedDot.dataset.hex : COLORS[0].hex;

  if(!start || !end){
    alert("Elige una fecha de inicio y una de fin.");
    return;
  }
  if(!activity){
  ...
  eachDateStr(start, end, (ds)=>{
    setEntry(state.brand, ds, { activity, notes, hex });
  });
  ...
  const el = document.getElementById(`month-${sy}-${sm-1}`);
...
function initEvents(){
  ...
  document.addEventListener("keydown", (e)=>{
    if(e.key === "Escape"){ closeModal(); closeRangeModal(); }
  });
  ...
  document.getElementById("rangeApplyBtn").addEventListener("click", applyRange);
  document.getElementById("rangeClearBtn").addEventListener("click", clearRange);
  document.getElementById("rangeOverlay").addEventListener("click", (e)=>{
    if(e.target.id === "rangeOverlay") closeRangeModal();
  });
}
